    ...
    <section id="how-it-works" className="py-24 px-6 bg-white">
      <div ref={sectionRef} className="section-hidden max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-16 relative">
          ...
          <div className="hidden md:block absolute top-8 left-[17%] right-[17%] h-px bg-[#e2e8f0] z-0" />

          {steps.map((step, i) => (
            <div key={i} className="stagger-card card-hidden bg-white border border-[#e2e8f0] shadow-sm rounded-xl p-8 relative z-10">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div className="text-2xl font-bold" style={{ color: "#2563eb", opacity: 0.4 }}>
                  {step.number}
                </div>
              </div>

              <h3 className="text-lg font-bold text-[#0f172a] mb-3 leading-tight">{step.title}</h3>
              <div
        ...
        {/* Weekly schedule */}
        <div className="border border-[#e2e8f0] rounded-2xl p-8 md:p-10" style={{ background: "#f8fafc" }}>
          <div className="text-center mb-8">
            <h3 className="text-xl font-bold text-[#0f172a] mb-2">Your weekly content schedule</h3>
            <p className="text-[#94a3b8] text-sm">Posts arrive in your inbox automatically</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            {schedule.map((item, i) => (
              <div key={i} className="text-center">
                ...
                <p className="text-[#0f172a] font-semibold text-sm mb-2">{item.day}</p>
                <div className="bg-white border border-[#e2e8f0] rounded-xl p-3 text-left shadow-sm">
                  <div className="flex items-center gap-1.5 mb-2">
                    <div className="w-1.5 h-1.5 rounded-full" style={{ background: "#22c55e" }} />
                    <span className="text-xs font-medium" style={{ color: "#22c55e" }}>Ready</span>
                  </div>
                  <p className="text-xs text-[#475569] leading-relaxed">{item.post}</p>
                  <div className="mt-2 flex items-center gap-1 text-xs" style={{ color: "#2563eb" }}>
